 ...
 */
public class RinexNavigationParserBds extends EphemerisSystemBds implements NavigationProducer {

    private final String TAG = this.getClass().getSimpleName();

    private ArrayList<EphBds> eph = new ArrayList<EphBds>(); /* GPS broadcast ephemerides */
    ...
    public RinexNavigationParserBds(EphemerisResponse ephResponse){
        for (GnssEphemeris eph : ephResponse.ephList) {
            if (eph instanceof BdsEphemeris) {
                this.eph.add(new EphBds((BdsEphemeris)eph));
            }
        }
     ...
     */
    public EphBds findEph(long unixTime, int satID, char satType) {
...
//        }
    ...
    public SatellitePosition getGpsSatPosition(Observations obs, int satID, char satType, double receiverClockError) {
        ...
        EphBds eph = findEph(unixTime, satID, satType);
        if (eph.equals(EphGps.UnhealthyEph))
            return SatellitePosition.UnhealthySat;

        if (eph != null) {
            //			char satType = eph.getSatType();
            SatellitePosition sp = computePositionBds(obs, satID, satType, eph, receiverClockError);
    ...
    public SatellitePosition getSatPositionAndVelocities(long unixTime, double range, int satID, char satType, double receiverClockError) {
        ...
        if (eph.equals(EphBds.UnhealthyEph))
            return SatellitePosition.UnhealthySat;
