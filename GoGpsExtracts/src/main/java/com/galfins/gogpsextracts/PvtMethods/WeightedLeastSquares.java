...
package com.galfins.gogpsextracts.PvtMethods;
...
import com.galfins.gogpsextracts.Constellations.Constellation;
...
public class WeightedLeastSquares extends PvtMethod {
    ...
    public Coordinates calculatePose(Constellation constellation) {
        ...
        if(CONSTELLATION_SIZE < 6){
            Coordinates pose = Coordinates.globalXYZInstance(0, 0, 6378137.0+21456.6);
            return pose;
        }
        int multi_sys=0;
        // Initialize matrices for data storage
        if(constellation.getName() == "BDS + GPS")
            multi_sys = 1;

        SimpleMatrix rxPosSimpleVector = new SimpleMatrix(5, 1);
        rxPosSimpleVector.set(0,constellation.getRxPos().getX());
        rxPosSimpleVector.set(1,constellation.getRxPos().getY());
        rxPosSimpleVector.set(2,constellation.getRxPos().getZ());
        rxPosSimpleVector.set(3,0);
        rxPosSimpleVector.set(4,0);
        //Constellation.getRxPosAsVector(constellation.getRxPos());
        ...
        int[] sysVect = new int[CONSTELLATION_SIZE];
        ...
        try {
            ...
            for (int ii = 0; ii < CONSTELLATION_SIZE; ii++) {
                ...
                //int satid = constellation.getSatellite(ii).getSatId();
                //int constell = constellation.getSatellite(ii).getConstellationType();
                prVect.set(ii, constellation.getSatellite(ii).getPseudorange());
                sysVect[ii] = constellation.getSatellite(ii).getConstellationType();
...
//                target.setXYZ(satPosMat.get(ii, 0),satPosMat.get(ii, 1),satPosMat.get(ii, 2) );
        ...
        try{
            ...
            SimpleMatrix xHat = new SimpleMatrix(4+multi_sys, 1); 			 // vector holding the WLS estimates
            SimpleMatrix z = new SimpleMatrix(CONSTELLATION_SIZE, 1);        // observation vector
            SimpleMatrix H = new SimpleMatrix(CONSTELLATION_SIZE, 4+multi_sys);        // observation matrix
            ...
            for (int iter = 0; iter < NUMBER_OF_ITERATIONS; iter++){
                ...
                if(multi_sys == 1) {
                    rxPosSimpleVector.set(4, xHat.get(4));
                }
            }
